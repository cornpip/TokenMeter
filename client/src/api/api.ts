...
export const getChats = async (roomId: number) => {
    const { data } = await api.get(`/rooms/${roomId}/chats`);
    return data;
};
export const getChatsBefore = (roomId: number) => api.get(`/rooms/${roomId}/chats`);

export const createChat = (chat: { time: string; message: string; sequence: number; is_answer: number; room_id: number }) => api.post('/chats', chat);
export const updateChat = (id: number, chat: { time: string; message: string; sequence: number; is_answer: number }) => api.put(`/chats/${id}`, chat);
